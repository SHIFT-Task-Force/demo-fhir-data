...
5. Whereas all the data pointing at Encounter and EpisodeOfCare should not have the sensitive tags propagated down, the propagation of sensitive tags in a Condition might be properly propagated down? For example during an Encounter, a blood-pressure and body-weight are likely captured, but they are not necessarily sensitive. It might be useful to make them available, rather than blocking them as sensitive just because they were taken during a sensitive encounter. But resources with a .reason element holding a Condition likely are as sensitive as that Condition. 
